...
- Registry не создаёт новый набор feature flags: он собирает существующую истину из runtime config, billing/quota, Safety restrictions, account state, image worker и sandbox.
...
- Приостановленный аккаунт всегда отображается как unavailable независимо от backend/config состояния.
- Chat и Development учитывают effective quota после billing/admin override и реальный остаток месячного local-compute budget.
