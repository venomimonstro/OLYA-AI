...
- Harness fail-closed: <10 аккаунтов, повторяющиеся session tokens/user_id, несовпадение build fingerprint, превышение p95/error-rate или отсутствие success дают non-zero exit.
...
- Отчёт `x1-real-load-acceptance-v2` атомарно записывается в `backups/load-acceptance-latest.json`; он содержит `git_head`, `source_fingerprint` и `target_build_fingerprint` и принимается Sprint85/86 только при точном совпадении кандидата.
