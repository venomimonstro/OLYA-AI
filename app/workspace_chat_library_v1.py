...
(function(){
 ...
 async function refresh(){try{const [rows,projects]=await Promise.all([call('/v1/conversations?limit=100&all_projects=true'),call('/v1/projects')]);rows.sort((a,b)=>(Number(!!b.pinned)-Number(!!a.pinned))||(new Date(b.updated_at)-new Date(a.updated_at)));history.replaceChildren();group(history,'Без проекта',rows.filter(c=>!c.project_id));const projectRoot=tree||history;if(tree)tree.replaceChildren();for(const p of projects){group(projectRoot,p.name,rows.filter(c=>c.project_id===p.id))}}catch(e){console.warn('OLYA chat library:',e)}}
