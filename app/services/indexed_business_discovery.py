...
_RATING_RE = re.compile(r"(?:рейтинг\s*)?([1-5](?:[.,]\d{1,2})?)\s*(?:из\s*5|★|звезд)?", re.I)
...
def _clean_name(hit: SearchHit) -> str:
    ...
    # Directory result titles commonly put business type/location after comma.
    if "," in value:
...
def _provider_for(url: str, gis_city: str, zoon_city: str) -> str:
    ...
    if host == "zoon.ru" and path.startswith(f"/{zoon_city}/"):
        # Exclude generic category/search pages: an indexed business card should
        # have at least two path components after the city namespace.
...
def _queries(question: str) -> tuple[str, ...]:
    gis_city, zoon_city, city_name = _city(question)
...
async def discover_indexed_businesses(question: str, discovery, *, limit: int = 10) -> list[MapPlace]:
    """Find concrete directory cards through the public search index.

    This deliberately mirrors the proven Yandex Medicine strategy: when a
    directory's own listing page blocks a VPS, indexed concrete card URLs are
    still useful evidence. No card URL is synthesized; only URLs returned by
    discovery and matching the requested city's directory namespace survive.
    """
