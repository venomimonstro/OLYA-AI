...
def _trip_user_abuse_breakers(db: Session, settings, now: datetime, *, persist: bool) -> list[dict[str,Any]]:
    threshold=max(1,int(getattr(settings,"public_launch_max_requests_per_user_hour",120)))
    since=now-timedelta(hours=1); rows=db.execute(select(UsageEvent.user_id,func.count()).where(UsageEvent.created_at>=since).group_by(UsageEvent.user_id).having(func.count()>threshold)).all(); opened=[]
    for user_id,count in rows:
        info={"user_id":user_id,"requests_last_hour":int(count),"threshold":threshold}
        opened.append(info)
...
def evaluate_public_launch(db: Session, settings, *, persist: bool=True) -> dict[str,Any]:
    blockers=[]; warnings=[]; capacity=read_capacity_report(settings)
    ...
    now=utcnow(); since=now-timedelta(hours=24); request_count=int(db.scalar(select(func.count(UsageEvent.id)).where(UsageEvent.created_at>=since)) or 0); failed_count=int(db.scalar(select(func.count(UsageEvent.id)).where(UsageEvent.created_at>=since,UsageEvent.success.is_(False))) or 0); failure_rate=failed_count/request_count if request_count else 0.0
...
def advance_rollout(db: Session,current:PublicRollout,*,exposure_percent:int,evaluation:dict[str,Any],actor_id:str|None)->PublicRollout:
    ...
    if exposure_percent<=current.exposure_percent:raise ValueError("New rollout exposure must be greater than current exposure")
...
def rollback_rollout(db:Session,current:PublicRollout,*,actor_id:str|None)->PublicRollout:
    target=db.get(PublicRollout,current.previous_rollout_id) if current.previous_rollout_id else None;target_percent=target.exposure_percent if target is not None else 0;current.state="rolled_back";current.rolled_back_at=utcnow();row=PublicRollout(version=_next_rollout_version(db),state="active" if target_percent>0 else "planned",exposure_percent=target_percent,assignment_salt=current.assignment_salt,baseline_metrics=current.baseline_metrics,latest_metrics=current.latest_metrics,guardrails=current.guardrails,decision={"status":"rollback","from_percent":current.exposure_percent,"to_percent":target_percent,"rolled_back_version":current.version},previous_rollout_id=current.id,created_by=actor_id,approved_by=actor_id,opened_at=utcnow() if target_percent>0 else None);db.add(row);db.flush();return row
