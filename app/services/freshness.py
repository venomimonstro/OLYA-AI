...
_RULES: tuple[tuple[FreshnessCategory, re.Pattern[str], int, int, str], ...] = (
    ...
    (
        "official_role",
        re.compile(
            r"\b(?:кто\s+(?:(?:сейчас|сегодня|ныне|на\s+данный\s+момент)\s+)?(?:(?:генеральный\s+)?директор|гендиректор|президент|премьер(?:-министр)?|министр|губернатор|мэр|ceo|cto|cfo)|"
            r"нынешн\w*\s+(?:(?:генеральный\s+)?директор|президент|ceo)|текущ\w*\s+(?:(?:генеральный\s+)?директор|президент|ceo)|"
            r"current\s+(?:president|prime\s+minister|ceo|director))\b",
            re.IGNORECASE,
        ),
        6 * 60 * 60,
        2,
