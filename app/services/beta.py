...
def calculate_beta_metrics(db: Session, *, cohort: str = "closed-beta-1", window_days: int = 30, now: datetime | None = None) -> dict:
    now = _aware(now) or _now()
    window_days = max(1, min(180, int(window_days)))
    since = now - timedelta(days=window_days)
    participants = list(db.scalars(select(BetaParticipant).where(BetaParticipant.cohort == cohort, BetaParticipant.enrolled_at <= now)).all())
    ...
    effective_participants = active_count + paused_count

    usage_window = list(db.scalars(select(UsageEvent).where(UsageEvent.user_id.in_(ids), UsageEvent.created_at >= since, UsageEvent.created_at <= now)).all()) if ids else []
    tasks = list(db.scalars(select(Task).where(Task.created_by.in_(ids), Task.created_at >= since, Task.created_at <= now)).all()) if ids else []
    frustration = list(db.scalars(select(FrustrationEvent).where(FrustrationEvent.user_id.in_(ids), FrustrationEvent.created_at >= since, FrustrationEvent.created_at <= now)).all()) if ids else []
    ...
    retention_usage = list(db.scalars(select(UsageEvent).where(UsageEvent.user_id.in_(ids), UsageEvent.created_at >= retention_since, UsageEvent.created_at <= now)).all()) if ids else []
    ...
    activated = sum(bool(usage_by_user.get(participant.user_id)) for participant in participants if participant.state != "removed")
    ...
    quality_rows = list(db.scalars(select(AnswerAudit).where(AnswerAudit.request_id.in_(request_ids))).all()) if request_ids else []
    ...
    quality_nonfailed_requests = sum(quality_by_request.get(request_id) in {"checked", "supported"} for request_id in request_ids)
    ...
    readiness = {
        "participant_target_met": 50 <= effective_participants <= 100,
        "task_target_met": len(tasks) >= 500,
        "capacity_recalibration_ready": 50 <= effective_participants <= 100 and len(tasks) >= 500,
    }
    return {
        "cohort": cohort, "window_days": window_days, "enrolled_count": len(participants),
        "active_participant_count": active_count, "paused_participant_count": paused_count, "removed_participant_count": removed_count,
        "activated_count": activated, "d1_eligible_count": d1e, "d1_retained_count": d1r,
        "d7_eligible_count": d7e, "d7_retained_count": d7r, "d30_eligible_count": d30e, "d30_retained_count": d30r,
        "request_count": request_count, "success_count": success, "task_count": len(tasks), "completed_task_count": completed,
        "frustration_count": len(frustration), "compute_minutes_total": round(cpu_minutes, 4),
        ...
        "metrics": metrics, "readiness": readiness, "measured_at": now,
    }
...
def build_beta_snapshot(db: Session, *, cohort: str = "closed-beta-1", window_days: int = 30, now: datetime | None = None) -> BetaSnapshot:
    ...
    row = BetaSnapshot(cohort=data["cohort"], window_days=data["window_days"], enrolled_count=data["enrolled_count"], activated_count=data["activated_count"], d1_eligible_count=data["d1_eligible_count"], d1_retained_count=data["d1_retained_count"], d7_eligible_count=data["d7_eligible_count"], d7_retained_count=data["d7_retained_count"], request_count=data["request_count"], success_count=data["success_count"], task_count=data["task_count"], completed_task_count=data["completed_task_count"], frustration_count=data["frustration_count"], compute_minutes_total=data["compute_minutes_total"], compute_minutes_per_active_user=data["compute_minutes_per_active_user"], p95_duration_ms=data["p95_duration_ms"], p95_queue_ms=data["p95_queue_ms"], metrics={**data["metrics"], "d30_eligible_count": data["d30_eligible_count"], "d30_retained_count": data["d30_retained_count"], "p50_duration_ms": data["p50_duration_ms"], "p99_duration_ms": data["p99_duration_ms"], "p50_queue_ms": data["p50_queue_ms"], "p99_queue_ms": data["p99_queue_ms"], "active_participant_count": data["active_participant_count"], "paused_participant_count": data["paused_participant_count"], "removed_participant_count": data["removed_participant_count"]}, readiness=data["readiness"])
    db.add(row); db.flush(); return row


def snapshot_dict(snapshot: BetaSnapshot) -> dict:
    data = {key: getattr(snapshot, key) for key in ("id", "cohort", "window_days", "enrolled_count", "activated_count", "d1_eligible_count", "d1_retained_count", "d7_eligible_count", "d7_retained_count", "request_count", "success_count", "task_count", "completed_task_count", "frustration_count", "compute_minutes_total", "compute_minutes_per_active_user", "p95_duration_ms", "p95_queue_ms", "metrics", "readiness", "created_at")}
    metrics = dict(snapshot.metrics or {})
    data.update({"d30_eligible_count": int(metrics.get("d30_eligible_count") or 0), "d30_retained_count": int(metrics.get("d30_retained_count") or 0), "p50_duration_ms": int(metrics.get("p50_duration_ms") or 0), "p99_duration_ms": int(metrics.get("p99_duration_ms") or 0), "p50_queue_ms": int(metrics.get("p50_queue_ms") or 0), "p99_queue_ms": int(metrics.get("p99_queue_ms") or 0), "active_participant_count": int(metrics.get("active_participant_count") or 0), "paused_participant_count": int(metrics.get("paused_participant_count") or 0), "removed_participant_count": int(metrics.get("removed_participant_count") or 0)})
    return data
