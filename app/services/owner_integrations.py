...
def integration_snapshot(db: Session, settings) -> dict:
    ...
    if row is None:
        return {
            ...
            "metrika": {"enabled": False, "ready": False, "counter_id": "", "webvisor": False},
            "payments": {
    ...
    yoomoney_ready = bool(
        row.yoomoney_enabled
        and row.yoomoney_receiver.strip()
        and row.yoomoney_notification_secret_ciphertext
        and base_ready
    )
    yookassa_ready = bool(
        row.yookassa_enabled
        and row.yookassa_shop_id.strip()
        and row.yookassa_secret_ciphertext
        and base_ready
    )
    ...
    return {
        ...
        "metrika": {
            ...
            "counter_id": counter,
            "webvisor": bool(row.metrika_webvisor),
            "goal_catalog": metrika_goal_catalog(),
        },
        "payments": {
            "default_provider": row.payment_default_provider,
            "yoomoney": {
                "enabled": bool(row.yoomoney_enabled),
                "ready": yoomoney_ready,
                "receiver": row.yoomoney_receiver,
                "secret_set": bool(row.yoomoney_notification_secret_ciphertext),
                "notification_path": "/v1/commerce/payments/yoomoney/notify",
            },
            "yookassa": {
                "enabled": bool(row.yookassa_enabled),
                "ready": yookassa_ready,
                "shop_id": row.yookassa_shop_id,
                "secret_set": bool(row.yookassa_secret_ciphertext),
                "webhook_path": "/v1/commerce/payments/yookassa/webhook",
            },
        },
...
def update_owner_settings(db: Session, settings, actor_id: str, payload: dict) -> OwnerIntegrationSettings:
    ...
    simple_fields = {
        ...
        "yoomoney_enabled", "yoomoney_receiver",
        "yookassa_enabled", "yookassa_shop_id",
...
def create_email_token(db: Session, user: User, *, purpose: str) -> tuple[str, AuthEmailToken]:
    ...
    row = AuthEmailToken(
        ...
        expires_at=now + timedelta(minutes=TOKEN_TTL_MINUTES[purpose]),
    )
    db.add(row)
    db.flush()
    return raw, row
...
def consume_email_token(db: Session, *, raw_token: str, purpose: str) -> User:
    ...
    row = db.scalar(
        select(AuthEmailToken).where(
            AuthEmailToken.token_hash == _token_hash(raw_token.strip()),
...
def send_verification_email(db: Session, settings, user: User) -> None:
    ...
    send_email(
        ...
        settings,
        to_email=user.email,
        subject="Подтвердите email для X1 AI",
        text=f"Подтвердите адрес электронной почты, чтобы пользоваться X1 AI:\n\n{url}\n\nСсылка действует 24 часа. Если вы не создавали аккаунт, письмо можно проигнорировать.",
    )
