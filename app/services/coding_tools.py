...
from app.services.code_workspace import (
    repo_map,
    resolve_inside,
    safe_relative_path,
    sha256_file,
...
def build_coding_tool_registry(root: Path, *, allowed_paths: list[str] | tuple[str, ...] = ()) -> ToolRegistry:
    """Create the minimal local coding tool set used by the Coding Agent.

    The registry intentionally does not expose shell, network, Git push, secrets,
    or arbitrary filesystem access. Those capabilities require separate server
    authorization boundaries and are not inferred from model output.
    """
    ...
    def workspace_map(args: WorkspaceMapArgs) -> dict:
        ...
        files = [
            ...
            if any(
                item.get("path") == base or str(item.get("path", "")).startswith(base.rstrip("/") + "/")
                for base in scope
            )
        ]
        return {
    ...
    def workspace_read(args: WorkspaceReadArgs) -> dict:
        rel = _require_allowed(args.path, scope)
        target = resolve_inside(workspace, rel)
        if target.is_symlink() or not target.is_file():
            raise ToolScopeError("Requested path is not a regular file")
        if target.stat().st_size > 2_000_000:
            raise ToolScopeError("File is too large for direct tool read")
        text = target.read_text(encoding="utf-8", errors="replace")
        lines = text.splitlines()
        start = min(args.start_line, len(lines) + 1)
        requested_end = args.end_line if args.end_line is not None else start + 399
        end = min(max(start, requested_end), min(len(lines), start + 999))
    ...
    def workspace_write(args: WorkspaceWriteArgs) -> dict:
        rel = _require_allowed(args.path, scope)
        return write_text(workspace, rel, args.content, expected_sha256=args.expected_sha256)

    def status(args: GitStatusArgs) -> dict:
        result = git_status(workspace)
        if args.include_diff:
            result["diff"] = git_diff(workspace, staged=args.staged)
        return result

    registry.register(
        ToolSpec(
            name="workspace.map",
            description="List files visible inside the approved coding workspace scope.",
            args_model=WorkspaceMapArgs,
            handler=workspace_map,
            effect="read",
            timeout_seconds=8,
            max_retries=1,
    ...
    registry.register(
        ToolSpec(
            name="workspace.read",
            description="Read a bounded line range from one approved workspace text file.",
    ...
    registry.register(
        ToolSpec(
            ...
            description="Atomically write one approved workspace file. expected_sha256 provides optimistic concurrency protection.",
    ...
    registry.register(
        ToolSpec(
            name="git.status",
            description="Inspect local Git status and optionally a bounded diff. This tool never commits or pushes.",
            args_model=GitStatusArgs,
            handler=status,
            effect="read",
            timeout_seconds=15,
            max_retries=1,
            result_max_chars=40_000,
        )
    )
    return registry
