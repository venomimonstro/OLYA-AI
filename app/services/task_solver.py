...
_CATEGORY_PATTERNS = (
    ...
    (("ресторан",), "ресторан"), (("кафе", "кофейн"), "кафе"),
    (("отел", "гостиниц"), "отель"), (("автосервис", "сто ", "ремонт авто"), "автосервис"),
    (("салон красоты", "парикмахер"), "салон красоты"), (("фитнес", "спортзал", "тренажер"), "фитнес-клуб"),
    (("юрист", "адвокат"), "юридическая компания"), (("школ", "курс"), "образовательный центр"),
)
_CITY_ALIASES = {
    ...
    "новосибирске": "Новосибирск", "новосибирск": "Новосибирск", "сочи": "Сочи",
    "уфе": "Уфа", "уфа": "Уфа", "перми": "Пермь", "пермь": "Пермь",
...
class TaskExecution:
    ...
    website_signals: list[dict] = field(default_factory=list)
...
def plan_task(question: str, *, max_queries: int = 4, force_web: bool = False) -> TaskSolvePlan:
    ...
    normalized = clean.casefold(); urls = _urls(clean); max_queries = max(1, min(int(max_queries), 6))
    if urls and any(marker in normalized for marker in _WEBSITE_AUDIT_MARKERS):
        target = urls[0]; host = (urlsplit(target).hostname or "").removeprefix("www.")
        queries = tuple(q for q in (f"site:{host}" if host else "", f'"{host}" SEO' if host else "") if q)[:max_queries]
        return TaskSolvePlan(
            kind="website_audit", requires_web=True, queries=queries, direct_urls=_origin_aux_urls(target),
            source_mix=("target_site", "robots", "sitemap", "indexed_pages"), max_sources=6, force_freshness=True,
            freshness_category="website_state", freshness_reason="Website audit must observe the current public site rather than rely on model memory.",
    ...
    location = _location(normalized); category = _category(normalized)
    if any(marker in normalized for marker in _RECOMMENDATION_MARKERS) and category and location:
        base = f"{category} {location}"
        queries = (base, f"{base} отзывы Яндекс 2ГИС ПроДокторов", f"{base} цены специалисты врачи", f"{base} лицензия официальный сайт")[:max_queries]
        return TaskSolvePlan(
            kind="local_recommendation", requires_web=True, queries=queries, location=location, category=category,
            source_mix=("official", "maps_catalog", "reviews", "pricing", "professional", "independent"), max_sources=6,
            force_freshness=True, freshness_category="local_business",
...
def diversify_hits(hits: list[SearchHit], *, kind: str, limit: int) -> list[dict]:
    ...
    enriched.sort(key=lambda row: (kind_priority.get(str(row.get("source_kind")), 1), float(row.get("discovery_score") or 0), -int(row.get("rank") or 999)), reverse=True)
    result: list[dict] = []; seen_urls: set[str] = set(); per_host: dict[str, int] = {}
    ...
    for row in enriched:
        ...
        host = _host(url); max_host = 2 if kind == "website_audit" else 1
        if host and per_host.get(host, 0) >= max_host: continue
        result.append(row); seen_urls.add(url); per_host[host] = per_host.get(host, 0) + 1
...
def _store_page(db: Session, *, user_id: str, project_id: str | None, page) -> ResearchSource:
    ...
    scope = [ResearchSource.user_id == user_id, ResearchSource.final_url == page.final_url, ResearchSource.content_sha256 == digest, ResearchSource.status == "ready"]
    ...
    if source is None:
        source = ResearchSource(user_id=user_id, project_id=project_id, url=page.requested_url, final_url=page.final_url, title=page.title,
                                content=page.content, content_sha256=digest, http_status=page.http_status, media_type=page.media_type, status="ready")
...
def _contract_message(execution: TaskExecution) -> ChatMessage:
    ...
    if execution.plan.kind == "local_recommendation":
        common += (
            ...
            "including official information, maps/catalogs, reviews and professional/price signals when available. Do not declare a decisive winner when evidence is weak or conflicting; "
            "give the strongest shortlist and explain the decision criteria. "
        )
    elif execution.plan.kind == "website_audit":
        common += (
    ...
    elif execution.plan.kind == "web_research":
...
def _website_signal_message(execution: TaskExecution) -> ChatMessage | None:
    if execution.plan.kind != "website_audit" or not execution.website_signals:
        return None
    blocks = [
        "OBSERVED TECHNICAL SEO SIGNALS FROM THE CURRENT PUBLIC SITE. Treat these as observations from fetched responses. "
        "Do not claim Core Web Vitals, Search Console data, traffic, rankings, JavaScript-rendered state or server configuration unless separately observed."
    ]
    for row in execution.website_signals[:6]:
        blocks.append(
            f"URL: {row.get('url')}\nresource={row.get('resource')} status={row.get('http_status')} media_type={row.get('media_type')}\n"
...
def _discovery_message(execution: TaskExecution, rows: list[dict]) -> ChatMessage | None:
    ...
    blocks = ["UNTRUSTED SEARCH-DISCOVERY SIGNALS. These help identify candidates but are weaker than fetched source snapshots. Do not treat snippets or search rank as verified facts."]
    for index, row in enumerate(rows[:12], start=1):
        blocks.append(f"[DISCOVERY {index}] kind={row.get('source_kind','web')} provider={row.get('provider','')} rank={row.get('rank','')}\nTitle: {str(row.get('title') or '')[:300]}\nURL: {row.get('url','')}\nSnippet: {str(row.get('snippet') or '')[:500]}")
    if execution.plan.kind == "local_recommendation":
        candidates = build_business_candidates(rows); summary = recommendation_summary(candidates)
        blocks.append("DETERMINISTIC COMPARISON SIGNAL (supporting aid, not final truth): " + ("evidence supports a potentially decisive leader." if summary["decisive_winner"] else "evidence does not justify an automatic decisive winner."))
        for candidate in candidates[:5]:
            blocks.append(f"Candidate: {candidate.display_name}; evidence={candidate.evidence_score:.2f}; rating={candidate.public_rating if candidate.public_rating is not None else 'n/a'}; review_count_signal={candidate.review_count_total}; independent_source_types={candidate.independent_source_count}; comparison={candidate.comparison_state}; conflicts={','.join(candidate.conflict_flags) or 'none'}")
    return ChatMessage(role="user", content="\n\n".join(blocks))
...
async def execute_task_solver(*, db: Session, user: User, settings, discovery, fetcher, question: str, project_id: str | None, force_web: bool = False) -> TaskExecution:
    ...
    for query in plan.queries:
        try:
            rows = await cached_provider_search(db, discovery, query, count=min(10, int(getattr(settings, "research_max_discovery_results", 20))), country="RU", language="ru", ttl_seconds=int(getattr(settings, "search_cache_ttl_seconds", 3600)), quality_mode=(plan.kind == "local_recommendation"))
    ...
    execution.discovered_hits = len(gathered); selected_rows = diversify_hits(gathered, kind=plan.kind, limit=plan.max_sources)
    ...
    async def fetch_one(url: str):
        async with semaphore: return url, await fetcher.fetch(url)

    results = await asyncio.gather(*(fetch_one(url) for url in urls), return_exceptions=True) if urls else []
    source_rows: list[ResearchSource] = []; failed = 0
    for result in results:
        if isinstance(result, asyncio.CancelledError): raise result
        if isinstance(result, BaseException): failed += 1; continue
        requested_url, page = result
    ...
    if discovery_message is not None: execution.context_messages.append(discovery_message)
    if plan.requires_web and not execution.source_ids: execution.warnings.append("Не удалось получить ни одного проверяемого снимка источника; итог должен явно обозначить ограничение")
    return execution
