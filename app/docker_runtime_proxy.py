...
def _managed(ref: str) -> bool:
    ...
    try:
        result = _raw(
            ["docker", "inspect", "-f", '{{ index .Config.Labels "x1.sandbox.preview" }}|{{ index .Config.Labels "x1.sandbox.execution" }}|{{ .Config.Image }}', ref],
            5,
        )
...
def _safe_mount(spec: str) -> bool:
    ...
    if target not in {"/workspace", "/x1-runtime"}:
        return False
    if target == "/workspace" and not ({"ro", "rw"} & flags):
        return False
...
def _validate_run(argv: list[str]) -> None:
    forbidden = {
        "--privileged", "--pid=host", "--ipc=host", "--uts=host", "--userns=host",
        "--device", "--volume", "-v", "--entrypoint", "--runtime", "--gpus",
    }
    if any(item in forbidden or item.startswith("--privileged=") for item in argv):
        raise HTTPException(status_code=422, detail="Forbidden Docker run option")
    if "--network" not in argv:
        raise HTTPException(status_code=422, detail="Sandbox run must define network policy")
    ...
    if not required.issubset(set(argv)):
        raise HTTPException(status_code=422, detail="Sandbox Docker hardening flags missing")
    if "--user" not in argv:
        raise HTTPException(status_code=422, detail="Sandbox container must run non-root")
    ...
    if not labels.intersection(_MANAGED_LABELS):
...
def _validate(argv: list[str]) -> None:
    ...
    if command == "image":
        ...
        return
    if command == "ps":
        allowed = {"docker", "ps", "-q", "--filter"}
        if len(argv) != 5 or not set(argv[:4]).issubset(allowed) or not argv[4].startswith("label=x1.sandbox."):
