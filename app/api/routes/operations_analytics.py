...
@router.get("/health")
async def health(request: Request, refresh: bool = Query(default=True), admin: User = Depends(require_admin), db: Session = Depends(get_db)) -> dict:
    _ = admin
    if refresh:
        result = await collect_system_health(request.app, db, persist=True); db.commit(); result["checkpoints"] = latest_checkpoints(db); return result
    return {"checkpoints": latest_checkpoints(db)}
