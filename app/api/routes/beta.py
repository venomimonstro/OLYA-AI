...
def enroll(user_id: str, payload: Enroll, admin: User = Depends(require_admin), db: Session = Depends(get_db)):
    ...
    if row is None:
        row = BetaParticipant(user_id=user_id, cohort=payload.cohort, source=payload.source, metadata_json=payload.metadata)
        db.add(row)
        db.flush()
    else:
        row.state = "active"
    ...
    audit(db, admin, "beta.enroll", "beta_participant", row.id, {"cohort": payload.cohort})
    db.commit()
    return {
        "id": row.id, "user_id": row.user_id, "cohort": row.cohort, "state": row.state,
        "source": row.source, "metadata": row.metadata_json, "enrolled_at": row.enrolled_at,
    }
...
def participants(
    ...
    return [
        {"id": row.id, "user_id": row.user_id, "state": row.state, "source": row.source, "enrolled_at": row.enrolled_at}
        for row in rows
    ]
