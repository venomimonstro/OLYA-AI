...
def create_generation(payload: ImageGenerationCreate, request: Request, user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> ImageGeneration:
    ...
    decision = evaluate_prompt(policy, payload.prompt)
    ...
    try:
        validate_dimensions(payload.width, payload.height, max_dimension=settings.image_max_dimension, max_pixels=settings.image_max_pixels)
    except ImageRuntimeError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
    if settings.image_backend == "disabled":
        raise HTTPException(status_code=503, detail="Local image generation is not configured")
    try:
    ...
    if int(active) >= settings.image_max_active_per_user:
        raise HTTPException(status_code=429, detail="Too many active image generations")
    ...
    job = enqueue_job(
        ...
        max_attempts=2,
        idempotency_key=f"image:{generation.id}",
    )
    generation.job_id = job.id
    generation.manifest = {
        "requested_backend": settings.image_backend,
        "requested_model": settings.image_model_name,
        "seed": seed,
        "steps": steps,
        "width": payload.width,
        "height": payload.height,
        "safety_policy_id": policy.id if policy else None,
        "safety_policy_version": policy.version if policy else None,
    }
    db.commit()
...
def image_status(request: Request, user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> dict:
    settings = request.app.state.settings
    active = int(db.scalar(select(func.count(ImageGeneration.id)).where(ImageGeneration.user_id == user.id, ImageGeneration.status.in_(["queued", "generating"]))) or 0)
    used = total_user_image_storage_bytes(db, user.id)
    configured = settings.image_backend != "disabled"
    quota_ok = used < settings.image_user_storage_quota_bytes
    slot_ok = active < settings.image_max_active_per_user
    worker = image_worker_snapshot(db, stale_seconds=90)
    worker_alive = bool(worker.get("alive"))
    reason = None
    if not configured:
        reason = "local_image_backend_not_configured"
    elif not worker_alive:
        reason = "image_worker_not_running"
    elif not quota_ok:
        reason = "storage_quota_reached"
    elif not slot_ok:
    ...
    # Qwen-Image-Edit may deliberately reuse the same Plus checkpoint for local
    # and identity/scene edits. Generic diffusers keeps the paths separate.
    identity_path = str(settings.image_edit_identity_model_path or "").strip()
    if str(settings.image_edit_backend).lower() == "qwen-image-edit" and not identity_path:
        identity_path = str(settings.image_edit_model_path or "").strip()
    identity_edit = edit_configured and bool(identity_path)
    vision_ready = bool(str(settings.image_vision_qa_url).strip())
    strict_qa = (not settings.image_edit_require_vision_qa) or vision_ready
    edit_reason = None
    if not edit_configured:
        edit_reason = "image_edit_backend_not_configured"
    elif not worker_alive:
        edit_reason = "image_worker_not_running"
    elif not strict_qa:
        edit_reason = "vision_qa_not_configured"
    elif not (local_edit or identity_edit):
        edit_reason = "image_edit_model_not_configured"
    elif not quota_ok:
        edit_reason = "storage_quota_reached"
    elif not slot_ok:
        edit_reason = "active_generation_limit"

    lanes = getattr(request.app.state, "overload_lanes", {}) or {}
    image_lane = lanes.get("images")
    queue_waiting = int(image_lane.snapshot().waiting) if image_lane is not None else 0
    return {
        "available": configured and worker_alive and quota_ok and slot_ok,
        "reason": reason,
        "backend": settings.image_backend if configured else "disabled",
        "active_generations": active,
        "max_active_generations": settings.image_max_active_per_user,
        "storage_used_bytes": used,
        "storage_quota_bytes": settings.image_user_storage_quota_bytes,
        "queue_waiting": queue_waiting,
        "worker": worker,
        "editing": {
            "available": edit_configured and worker_alive and quota_ok and slot_ok and strict_qa and (local_edit or identity_edit),
            "reason": edit_reason,
            "backend": settings.image_edit_backend if edit_configured else "disabled",
            "local_object_edit": local_edit,
            "identity_recompose": identity_edit,
            "strict_quality_ready": strict_qa,
            "automatic_object_localization": vision_ready,
            "vision_qa_url_kind": "configured_internal" if vision_ready else "disabled",
            "max_source_bytes": settings.image_edit_max_source_bytes,
            "max_source_dimension": settings.image_edit_max_source_dimension,
        },
    }


@router.get("/generations", response_model=list[ImageGenerationRead])
def list_generations(user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> list[ImageGeneration]:
    return list(db.scalars(select(ImageGeneration).where(ImageGeneration.user_id == user.id).order_by(ImageGeneration.created_at.desc()).limit(100)).all())


@router.get("/generations/{generation_id}", response_model=ImageGenerationRead)
def get_generation(generation_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> ImageGeneration:
    return _access(db, user, generation_id)
...
def cancel_generation(generation_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> ImageGeneration:
    ...
    if row.user_id != user.id and row.project_id:
    ...
    if row.status == "generating":
        raise HTTPException(status_code=409, detail="Generation is already running; cooperative cancellation is not available for this backend yet")
    return row
...
def image_content(generation_id: str, variant: str = Query(default="preferred", pattern="^(preferred|source|preview)$"), user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> Response:
    ...
    if row.delivery_status != "active":
        raise HTTPException(status_code=451 if row.delivery_status == "revoked" else 423, detail="Image delivery is not available")
    if row.status != "ready" or row.qa_status != "passed" or not row.blob_id:
        raise HTTPException(status_code=409, detail="Image has not passed quality assurance")
    ...
    if blob is None:
        raise HTTPException(status_code=500, detail="Image blob missing")
    ...
    return FileResponse(
        ...
        headers={"ETag": blob.sha256, "Cache-Control": "private, max-age=31536000, immutable", "X-X1-Image-Variant": variant},
    )
...
class ImageFeedbackWrite(BaseModel):
...
def image_feedback(generation_id: str, payload: ImageFeedbackWrite, user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> Response:
    ...
    if row.user_id != user.id:
        raise HTTPException(status_code=403, detail="Only the generation owner can submit training consent")
    fb = db.scalar(select(ImageFeedback).where(ImageFeedback.generation_id == row.id, ImageFeedback.user_id == user.id))
    if fb is None:
        fb = ImageFeedback(generation_id=row.id, user_id=user.id, rating=payload.rating, allow_training=payload.allow_training)
        db.add(fb)
    else:
        fb.rating = payload.rating
        fb.allow_training = payload.allow_training
    db.flush()
