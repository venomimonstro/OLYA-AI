...
def create_generation(payload: ImageGenerationCreate, request: Request, user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> ImageGeneration:
    ...
    if user_image_storage_bytes(db, user.id) >= settings.image_user_storage_quota_bytes:
    ...
    generation = ImageGeneration(user_id=user.id, project_id=payload.project_id, prompt=payload.prompt.strip(), negative_prompt=payload.negative_prompt.strip(), status="queued", backend=settings.image_backend, model_name=settings.image_model_name, width=payload.width, height=payload.height, steps=steps, seed=seed, safety_policy_id=policy.id if policy else None, safety_status="allowed" if policy else "no_policy", delivery_status="active")
    db.add(generation); db.flush()
    job = enqueue_job(db, kind="image.generate", payload={"generation_id": generation.id}, user_id=user.id, project_id=payload.project_id, priority=settings.image_job_priority, max_attempts=2, idempotency_key=f"image:{generation.id}")
    generation.job_id = job.id
    generation.manifest = {"requested_backend": settings.image_backend, "requested_model": settings.image_model_name, "seed": seed, "steps": steps, "width": payload.width, "height": payload.height, "safety_policy_id": policy.id if policy else None, "safety_policy_version": policy.version if policy else None}
    db.commit(); db.refresh(generation); return generation
...
def image_feedback(generation_id: str, payload: ImageFeedbackWrite, user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> Response:
    ...
    feedback = db.scalar(select(ImageFeedback).where(ImageFeedback.generation_id == row.id, ImageFeedback.user_id == user.id))
    if feedback is None:
        feedback = ImageFeedback(generation_id=row.id, user_id=user.id, rating=payload.rating, allow_training=payload.allow_training)
        db.add(feedback)
    else:
        feedback.rating = payload.rating
        feedback.allow_training = payload.allow_training
    db.flush()
    if payload.allow_training:
        build_candidate(db, row, feedback)
    else:
        withdraw_training_consent(db, row.id, user.id)
    db.commit()
