...
from app.models import Conversation, ConversationMemory, ProjectMemory, User
...
project_router = APIRouter(prefix="/v1/projects/{project_id}/memory", tags=["memory"])
_MEMORY_KINDS = ("explicit", "decision", "preference", "fact")


@project_router.get("", response_model=list[MemoryResponse])
...
@project_router.put("", response_model=MemoryResponse)
...
@project_router.delete("/{memory_id}", status_code=status.HTTP_204_NO_CONTENT)
...
def list_user_memory(
    ...
    rows = list(
        db.execute(
            ...
            .where(Conversation.owner_id == user.id, ConversationMemory.kind.in_(_MEMORY_KINDS))
