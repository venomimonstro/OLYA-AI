...
from app.services.reliability import collect_system_health, latest_checkpoints
...
async def reliability_status(
    ...
    if refresh:
        ...
        db.commit()
        result["checkpoints"] = latest_checkpoints(db)
        return result
    checkpoints = latest_checkpoints(db)
...
def reliability_history(
    ...
    return [{
        "id": x.id, "status": x.overall_status, "score": x.score,
        "stable": x.stable_count, "degraded": x.degraded_count, "failed": x.failed_count,
        "critical_failed": x.critical_failed_count, "created_at": x.created_at,
    } for x in rows]
