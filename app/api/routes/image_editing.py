...
from app.services.image_editing import ImageEditError, infer_edit_mode, resolve_edit_mode
...
def generation_as_reference(
    ...
    if generation.project_id:
        require_project_role(db, user, generation.project_id, "viewer")
...
def create_edit(
    ...
    if resolved_mode == "identity_recompose" and not str(settings.image_edit_identity_model_path).strip():
        raise HTTPException(status_code=503, detail="Identity-preserving scene editing model is not configured")
    if resolved_mode != "identity_recompose" and resolved_mode != "auto" and not str(settings.image_edit_model_path).strip():
        raise HTTPException(status_code=503, detail="Local inpainting model is not configured")
    if payload.strict_quality and settings.image_edit_require_vision_qa and not str(settings.image_vision_qa_url).strip():
...
def list_edits(user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> list[ImageEditRequest]:
    return list(db.scalars(select(ImageEditRequest).where(ImageEditRequest.user_id == user.id).order_by(ImageEditRequest.created_at.desc()).limit(100)).all())
