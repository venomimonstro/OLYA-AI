...
class OwnerIntegrationUpdate(BaseModel):
    ...
    yandex_oauth_enabled: bool | None = None
    yandex_oauth_client_id: str | None = Field(default=None, max_length=160)
    yandex_oauth_client_secret: str | None = Field(default=None, max_length=1000)
...
def put_integrations(payload: OwnerIntegrationUpdate, request: Request, admin: User = Depends(require_admin), db: Session = Depends(get_db)) -> dict:
    ...
    safe_fields = sorted(k for k in data if not k.endswith(("password", "secret")))
    audit(db, admin, "owner.integrations.update", "owner_integration_settings", "global", {"fields": safe_fields})
    db.commit()
    return integration_snapshot(db, request.app.state.settings)


@router.post("/v1/admin/integrations/smtp/test")
def test_smtp(payload: SMTPTestRequest, request: Request, admin: User = Depends(require_admin), db: Session = Depends(get_db)) -> dict:
    recipient = (payload.recipient or admin.email).strip().lower()
    try:
        send_email(db, request.app.state.settings, to_email=recipient, subject="X1 AI — проверка SMTP", text="SMTP настроен корректно. Это тестовое письмо из панели владельца X1 AI.")
...
def _owner_finance(db: Session, settings, since: datetime) -> dict:
    ...
    active_subs = list(db.scalars(select(BillingSubscription).where(BillingSubscription.status == "active", BillingSubscription.current_period_end > now, BillingSubscription.plan != "free")).all())
    ...
    refunds_minor = paid_minor = 0
    for row in payments:
        p = by_provider.setdefault(str(row.provider), {"records": 0, "payments_minor": 0, "refunds_minor": 0})
        p["records"] += 1
        if row.kind == "refund":
            p["refunds_minor"] += int(row.amount_minor or 0); refunds_minor += int(row.amount_minor or 0)
        else:
            p["payments_minor"] += int(row.amount_minor or 0)
            if row.status in {"applied", "paid", "reconciled", "succeeded"}: paid_minor += int(row.amount_minor or 0)
    ...
    return {
        "mrr_rub": mrr_rub, "active_paid_users": paid_users, "active_paid_by_plan": plan_counts,
        "arppu_rub": arppu, "arpu_rub": arpu, "server_cost_rub": server_cost,
        ...
        "checkout_created": len(checkouts), "checkout_paid": checkout_counts.get("paid", 0),
        "checkout_conversion": round(checkout_counts.get("paid", 0) / max(1, len(checkouts)), 4),
        "checkout_by_status": checkout_counts, "recognized_payment_rub_window": round(paid_minor / 100, 2),
        "refund_rub_window": round(refunds_minor / 100, 2), "payments_by_provider": by_provider,
    }


@router.get("/v1/admin/owner-dashboard")
def owner_dashboard(request: Request, days: int = Query(default=30, ge=1, le=365), admin: User = Depends(require_admin), db: Session = Depends(get_db)) -> dict:
    _ = admin
    ...
    return {
        ...
        "accounts": {"total_users": total_users, "new_users": new_users, "active_24h": active_24h, "active_7d": active_7d, "activation_rate": product.get("activation", {}).get("activation_rate"), "d1_retention": product.get("retention", {}).get("d1", {}).get("rate"), "d7_retention": product.get("retention", {}).get("d7", {}).get("rate")},
        "finance": _owner_finance(db, request.app.state.settings, since),
        "quality": {"request_success_rate": ops.get("traffic", {}).get("success_rate"), "frustration_rate": ops.get("traffic", {}).get("frustration_rate"), "task_success_rate": product.get("task_success", {}).get("terminal_success_rate"), "compute_waste_rate": ops.get("compute_economics", {}).get("waste_rate")},
        "performance": {"p95_duration_ms": ops.get("traffic", {}).get("p95_duration_ms"), "p95_queue_ms": ops.get("traffic", {}).get("p95_queue_ms"), "cpu_seconds_per_success": ops.get("traffic", {}).get("cpu_seconds_per_success"), "inference_minutes": ops.get("traffic", {}).get("inference_minutes"), **{k: v for k, v in (ops.get("resources") or {}).items() if not isinstance(v, (dict, list))}},
        "support": {"open": sum(int(tickets.get(x, 0)) for x in ("open", "in_progress", "waiting_user", "waiting_admin")), "waiting_admin": int(tickets.get("open", 0)) + int(tickets.get("waiting_admin", 0)), "by_status": {str(k): int(v) for k, v in tickets.items()}},
        "integrations": integrations, "metrika_goals": integrations.get("metrika", {}).get("goal_catalog", []),
    }
