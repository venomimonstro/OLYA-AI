...
def _local_business_metadata(execution) -> dict:
    ...
    return {
        ...
        "steps": ["Проверяю локальный индекс OLYA", "Обновляю через OpenStreetMap, Яндекс, 2ГИС, Zoon и Yell", "Возвращаю подтверждённые организации"],
    }
...
async def _smart_managed_runner(payload: ChatRequest, request: Request, user_id: str, job: ActiveChatJob) -> ChatResponse:
    """Latency-aware path with local-business resilience and bounded completeness repair."""
    with SessionLocal() as job_db:
        ...
        try:
            ...
            if not atomic and needs_expansion(question, result.text):
                ...
                # Keep collected evidence available during the repair, but the editor
                # is explicitly forbidden from inventing new facts or sources.
