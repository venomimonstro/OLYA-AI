...
def _validate_host_budget(payload: RuntimeCreate, settings) -> None:
    violations: dict[str, dict[str, float | int]] = {}
    if payload.cpu_limit > float(settings.project_runtime_max_cpu_limit):
        violations["cpu_limit"] = {"requested": payload.cpu_limit, "maximum": settings.project_runtime_max_cpu_limit}
    if payload.memory_limit_mb > int(settings.project_runtime_max_memory_mb):
        violations["memory_limit_mb"] = {"requested": payload.memory_limit_mb, "maximum": settings.project_runtime_max_memory_mb}
    if payload.process_limit > int(settings.project_runtime_max_process_limit):
        violations["process_limit"] = {"requested": payload.process_limit, "maximum": settings.project_runtime_max_process_limit}
    if violations:
        raise HTTPException(
            status_code=422,
            detail={
                "code": "runtime_host_budget_exceeded",
                "message": "Requested project runtime is above the production host safety envelope",
...
def create_runtime(payload: RuntimeCreate, request: Request, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    _validate_host_budget(payload, request.app.state.settings)
    ...
    try:
        ...
        rt.manifest = build_manifest(
            Path(ws.root_path),
            project_id=ws.project_id,
            workspace_id=ws.id,
            cpu_limit=rt.cpu_limit,
            memory_mb=rt.memory_limit_mb,
            disk_mb=rt.disk_limit_mb,
            process_limit=rt.process_limit,
            network_policy=rt.network_policy,
        ) | {"isolation": isolation, "git": git_state}
        db.add(
            GitRepositoryBinding(
                ...
                created_by=user.id,
                provider="local",
                default_branch=git_state.get("branch") or "main",
                mode="local_only",
                status="ready",
                last_local_head=git_state.get("head", ""),
            )
        )
        db.commit()
    except Exception:
...
def snapshot_runtime(runtime_id: str, request: Request, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    existing = db.scalar(
        select(ProjectRuntimeSnapshot).where(
            ProjectRuntimeSnapshot.runtime_id == rt.id,
...
def list_snapshots(runtime_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return list(
        db.scalars(
            select(ProjectRuntimeSnapshot)
            .where(ProjectRuntimeSnapshot.runtime_id == rt.id)
            .order_by(ProjectRuntimeSnapshot.created_at.desc())
        ).all()
    )
...
def put_secret(runtime_id: str, payload: RuntimeSecretWrite, request: Request, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    row = db.scalar(
        select(ProjectRuntimeSecret).where(
            ProjectRuntimeSecret.runtime_id == rt.id,
            ProjectRuntimeSecret.name == payload.name,
        )
    )
    if row is None:
...
def list_secrets(runtime_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return list(
        db.scalars(
            select(ProjectRuntimeSecret)
            .where(ProjectRuntimeSecret.runtime_id == rt.id)
            .order_by(ProjectRuntimeSecret.name)
        ).all()
    )
