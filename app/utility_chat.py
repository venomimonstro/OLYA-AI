...
_LOCATION_REQUIRED_RE = re.compile(
    r"^(?:"
    r"где\s+(?:поесть|покушать|перекусить|выпить\s+кофе)(?:\s+.{1,30})?\s+(?:рядом|поблизости)(?:\s+со\s+мной)?|"
    r"(?:кафе|ресторан\w*|кофейн\w*|аптек\w*|отел\w*|гостиниц\w*)\s+(?:рядом|поблизости)(?:\s+со\s+мной)?|"
    r"куда\s+сходить(?:\s+(?:сегодня|вечером|сейчас|щас))?|"
    r"что\s+посмотреть\s+(?:рядом|поблизости)|"
    r"(?:where\s+to\s+eat|restaurant|cafe|hotel|pharmacy)\s+near\s+me|"
    r"where\s+should\s+i\s+go(?:\s+(?:today|tonight))?"
    r")\s*[?!.…]*$",
    re.I,
)
_CITY_TIMEZONES = {
