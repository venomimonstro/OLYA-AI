...
function owner(o){const root=$('owner');clear(root);const f=o.finance||{},a=o.accounts||{},q=o.quality||{},s=o.support||{},i=o.integrations||{};root.append(metric('MRR',rub(f.mrr_rub),Number(f.mrr_after_server_rub||0)>=0?'ok':'warn'),metric('MRR − server',rub(f.mrr_after_server_rub),Number(f.mrr_after_server_rub||0)>=0?'ok':'bad'),metric('paid users',f.active_paid_users),metric('ARPPU',rub(f.arppu_rub)),metric('activation',pct(a.activation_rate)),metric('D7 retention',pct(a.d7_retention)),metric('request success',pct(q.request_success_rate)),metric('support ждёт',s.waiting_admin,Number(s.waiting_admin||0)>0?'warn':'ok'),metric('SMTP',i.smtp?.ready?'ready':'not ready',i.smtp?.ready?'ok':'bad'),metric('payment',i.payments?.yookassa?.ready||i.payments?.yoomoney?.ready?'ready':'not ready',i.payments?.yookassa?.ready||i.payments?.yoomoney?.ready?'ok':'bad'))}
