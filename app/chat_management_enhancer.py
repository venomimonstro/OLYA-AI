...
async function renameConversation(c){const name=window.prompt('Название чата',c.title||'Новый чат');if(name===null)return;const title=name.trim();if(!title||title===c.title)return;try{await api('/v1/conversations/'+encodeURIComponent(c.id),{method:'PATCH',body:JSON.stringify({title})},30000);await loadConversations()}catch(e){state.textContent=e.message}}
...
async function deleteConversation(c){if(!window.confirm('Удалить чат «'+(c.title||'Новый чат')+'»? Это действие нельзя отменить.'))return;try{await api('/v1/conversations/'+encodeURIComponent(c.id),{method:'DELETE'},30000);if(c.id===conversationId)newChat();else await loadConversations()}catch(e){state.textContent=e.message}}
async function quickCreateProject(){const value=window.prompt('Название нового проекта');if(value===null)return;const name=value.trim();if(!name)return;try{const p=await api('/v1/projects',{method:'POST',body:JSON.stringify({name,description:'',instructions:''})},30000);await loadProjects();await loadConversations();$('project-chat').value=p.id;newChat()}catch(e){state.textContent=e.message}}
async function deleteProjectFolder(p){if(!window.confirm('Удалить проект «'+p.name+'»? Чаты сохранятся и вернутся в «Без проекта». Файлы и память проекта будут удалены.'))return;try{await api('/v1/projects/'+encodeURIComponent(p.id),{method:'DELETE'},30000);if($('project-chat').value===p.id)$('project-chat').value='';if(workspaceProjectId===p.id)closeProjectWorkspace();await loadProjects();await loadConversations()}catch(e){state.textContent=e.message}}'''
