...
def test_failed_usage_does_not_fake_success(register_user, db_session):
    ...
    db_session.add(
        UsageEvent(
            user_id=data["user_id"], project_id=None, conversation_id=conversation.id,
            mode="work", raw_chars=20, compiled_chars=20, output_chars=5,
            duration_ms=500, inference_ms=400, queue_ms=0, success=False, request_id=run.id,
        )
    )
    db_session.commit()
    db_session.expire_all()
...
def test_successful_chat_run_is_sticky_against_late_cancel_or_failure(register_user, db_session):
    ...
    db_session.add(
        UsageEvent(
            user_id=data["user_id"], project_id=None, conversation_id=conversation.id,
            mode="fast", raw_chars=10, compiled_chars=10, output_chars=10,
            duration_ms=100, inference_ms=80, queue_ms=0, success=True, request_id=run.id,
        )
    )
    db_session.commit()
