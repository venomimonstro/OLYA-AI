...
def test_capacity_report_rejects_stale_and_accepts_current(tmp_path: Path):
    ...
    assert read_capacity_report(cfg)["status"] == "passed"
...
def test_capacity_recommendation_selects_largest_passing_tier():
    ...
    plan = recommendation(candidates, 180)
    assert plan["deep_context_tokens"] == 12288 and plan["max_concurrent_generations"] == 1
...
def test_beta_capacity_plan_uses_measured_compute_only_after_data_gate(tmp_path: Path):
    ...
    capacity = {"status": "passed", "recommendation": {"deep_context_tokens": 12288, "max_concurrent_generations": 1, "max_queue_size": 6, "inference_queue_timeout_seconds": 100}}
...
def test_insufficient_beta_never_freezes_measured_limits(tmp_path: Path):
    cfg = settings(tmp_path)
    beta = {"enrolled_count": 12, "task_count": 40, "request_count": 80, "compute_minutes_per_active_user": 50, "p95_queue_ms": 0, "metrics": {"request_success_rate": 1.0, "frustration_per_request": 0.0}, "readiness": {"capacity_recalibration_ready": False}}
    capacity = {"status": "passed", "recommendation": {"deep_context_tokens": 8192, "max_concurrent_generations": 1, "max_queue_size": 4}}
    result = build_launch_calibration(beta, capacity, cfg)
    assert result["status"] == "collecting_data" and result["plan"]["default_monthly_compute_seconds"] == cfg.default_monthly_compute_seconds


def test_sprint36_admin_routes_registered():
    paths = {getattr(route, "path", "") for route in app.routes}
    assert "/v1/admin/beta/current" in paths and "/v1/admin/beta/calibration" in paths
    assert app.version == "0.36.0"
