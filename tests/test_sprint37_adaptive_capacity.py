...
def settings(**overrides):
    data = {
        "beta_wave_min_observation_requests": 50,
        "beta_min_request_success_rate": 0.97,
        "beta_max_frustration_per_request": 0.05,
        "beta_max_p95_queue_ms": 5000,
        "beta_wave_max_queue_regression_ratio": 1.5,
        "beta_wave_max_duration_regression_ratio": 1.5,
        "beta_wave_min_cpu_efficiency_ratio": 0.70,
        "beta_trend_max_success_drop": 0.02,
...
def snapshot(
    *,
    ...
    return {
        ...
        "metrics": {
            "request_success_rate": successes / requests if requests else 0,
            "frustration_per_request": frustration / requests if requests else 0,
            "verified_request_success_per_cpu_minute": efficiency,
            "completed_task_success_per_cpu_minute": 1.0,
            "quality_supported_request_rate": 0.9,
            "d1_retention": 0.5,
            "d7_retention": 0.4,
            "d30_retention": 0.3,
        },
...
def test_wave_guardrail_pauses_on_new_request_failures():
    baseline = snapshot(requests=100, successes=99, frustration=1)
    current = snapshot(requests=150, successes=139, frustration=2)
    result = compare_wave_signals(current, baseline, settings())
    assert "wave_request_success_below_guardrail" in result["regressions"]


def test_wave_guardrail_detects_queue_regression():
    baseline = snapshot(requests=100, successes=99, frustration=1, queue=1000)
    current = snapshot(requests=150, successes=149, frustration=1, queue=6000)
    result = compare_wave_signals(current, baseline, settings())
...
def test_capacity_plan_approval_is_fail_closed():
    plan = SimpleNamespace(
        status="draft",
        guardrails={"calibration_status": "collecting_data", "blockers": ["beta_tasks_below_minimum"]},
        approved_by=None,
        approved_at=None,
    )
    with pytest.raises(ValueError):
        approve_plan(plan, actor_id="admin")
...
def test_live_safe_plan_can_reduce_limits_without_restart(tmp_path):
    runtime_settings = SimpleNamespace(
        ...
        deep_context_tokens=16384,
        max_concurrent_generations=1,
        max_queue_size=64,
        inference_queue_timeout_seconds=120.0,
    ...
    fake_app = SimpleNamespace(
        state=SimpleNamespace(
            ...
            capacity_boot_max_context_tokens=8192,
            capacity_boot_deep_context_tokens=16384,
            capacity_boot_max_concurrent_generations=1,
            context=SimpleNamespace(max_chars=16384 * 6),
            governor=SimpleNamespace(max_queue=64, wait_timeout_seconds=120.0),
        )
    )
    plan = SimpleNamespace(
        version=2,
        plan={
            "max_context_tokens": 8192,
            "deep_context_tokens": 12288,
            "max_concurrent_generations": 1,
            "max_queue_size": 8,
            "inference_queue_timeout_seconds": 90.0,
            "default_monthly_compute_seconds": 900,
        },
        requires_restart=True,
        runtime_applied=False,
    )
    result = apply_live_safe(fake_app, plan)
    assert result["runtime_applied"] is True
    assert runtime_settings.deep_context_tokens == 12288
    assert fake_app.state.governor.max_queue == 8
...
def test_plan_above_boot_ceiling_requires_restart(tmp_path):
    ...
    fake_app = SimpleNamespace(
        state=SimpleNamespace(
            settings=runtime_settings,
            capacity_boot_max_context_tokens=8192,
            capacity_boot_deep_context_tokens=8192,
            capacity_boot_max_concurrent_generations=1,
            context=SimpleNamespace(max_chars=8192 * 6),
            governor=SimpleNamespace(max_queue=8, wait_timeout_seconds=90.0),
        )
    )
    plan = SimpleNamespace(
        version=3,
        plan={"max_context_tokens": 8192, "deep_context_tokens": 16384, "max_concurrent_generations": 1},
    ...
    assert runtime_settings.deep_context_tokens == 8192
...
def test_plan_diff_is_explicit_and_version_friendly():
    ...
    assert result["changes"]["deep_context_tokens"] == {"from": 8192, "to": 12288}


def test_trend_anomaly_detects_quality_and_latency_regression():
    previous = snapshot(requests=100, successes=99, frustration=1, queue=1000, duration=20000, efficiency=1.0)
    current = snapshot(requests=120, successes=115, frustration=6, queue=3000, duration=40000, efficiency=0.5)
    current["id"] = "new"
    previous["id"] = "old"
    current["metrics"]["quality_supported_request_rate"] = 0.7
    previous["metrics"]["quality_supported_request_rate"] = 0.9
    result = compare_snapshots(current, previous, settings())
    assert result["status"] == "regressed"
    assert "quality_supported_rate_regressed" in result["anomalies"]
    assert "p95_queue_regressed" in result["anomalies"]
    assert "verified_cpu_efficiency_regressed" in result["anomalies"]


def test_sprint37_routes_and_version_are_registered():
    ...
    for path in (
        ...
        "/v1/admin/beta/feedback",
        "/v1/admin/beta/capacity-plans/propose",
        "/v1/admin/beta/capacity-plans/rollback",
    ):
        assert path in paths


def test_sprint37_has_single_alembic_head():
    config = Config("alembic.ini")
    heads = ScriptDirectory.from_config(config).get_heads()
    assert heads == ["f37e6b04a325"]
