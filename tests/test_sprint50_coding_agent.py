...
from app.services.coding_agent import (
    ...
    _Checkpoint,
    _diff_line_count,
    _diff_was_inspected_after_last_write,
    _latest_passed,
    _patch_digest,
)
...
def test_non_cacheable_verification_tool_reexecutes():
    ...
    registry.register(ToolSpec(name="verification.test", description="verify", args_model=EmptyArgs, handler=handler, max_retries=0, cacheable=False))
    session = ToolSession(registry, max_same_call=4)
    first = run(session.execute(ToolCall(name="verification.test", arguments={}, call_id="v1")))
    second = run(session.execute(ToolCall(name="verification.test", arguments={}, call_id="v2")))
...
def test_checkpoint_restores_existing_and_removes_new_file(tmp_path: Path):
    ...
    old = root / "a.py"; old.write_text("old\n", encoding="utf-8")
    new = root / "b.py"
    cp = _Checkpoint()
    cp.capture("a.py", old); cp.mark_touched({"path":"a.py"})
    cp.capture("b.py", new); cp.mark_touched({"path":"b.py"})
...
def test_patch_digest_changes_after_post_test_edit(tmp_path: Path):
    ...
    cp = _Checkpoint(); cp.capture("a.py", target); cp.mark_touched({"path":"a.py"})
    first = _patch_digest(root, cp)
    evidence = [VerificationEvidence(check="unit", argv=("pytest",), passed=True, exit_code=0, timed_out=False, sandbox_level="test", patch_digest=first, stdout_tail="", stderr_tail="")]
    assert _latest_passed(evidence, "unit", first) is True
    target.write_text("a=2\n", encoding="utf-8")
    second = _patch_digest(root, cp)
    assert second != first
    assert _latest_passed(evidence, "unit", second) is False
...
def test_final_diff_must_be_after_last_write():
    bad = (
        {"tool":"proof.inspect_diff","status":"ok"},
        {"tool":"workspace.write","status":"ok"},
    )
    good = (
        {"tool":"workspace.write","status":"ok"},
        {"tool":"verification.run","status":"ok"},
        {"tool":"proof.inspect_diff","status":"ok"},
    )
...
def test_sandbox_remote_contract_carries_read_only_workspace():
    client = Path("app/services/sandbox.py").read_text(encoding="utf-8")
    worker = Path("app/sandbox_worker_api.py").read_text(encoding="utf-8")
