...
ENV_COPY="$(mktemp -t x1-env.XXXXXX)"
...
ENV_EXISTED=0
if [ -f .env ]; then cp -p .env "$ENV_COPY"; ENV_EXISTED=1; else : > "$ENV_COPY"; fi
BACKUP_PATH=""

cleanup_helpers() { rm -f "$BACKUP_COPY" "$DRILL_COPY" "$RESTORE_COPY" "$MIGRATE_COPY" "$ENV_COPY"; }
