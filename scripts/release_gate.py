...
def run(name: str, argv: list[str], *, timeout: int, required: bool = True, env: dict[str, str] | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        return {"name": name, "status": "passed" if completed.returncode == 0 else "failed", "required": required, "exit_code": completed.returncode, "duration_seconds": round((datetime.now(timezone.utc) - started).total_seconds(), 3), "stdout": completed.stdout[-8000:], "stderr": completed.stderr[-8000:]}
    except subprocess.TimeoutExpired as exc:
        return {"name": name, "status": "failed", "required": required, "exit_code": None, "duration_seconds": round((datetime.now(timezone.utc) - started).total_seconds(), 3), "stdout": (exc.stdout or "")[-8000:] if isinstance(exc.stdout, str) else "", "stderr": (exc.stderr or "")[-8000:] if isinstance(exc.stderr, str) else "", "error": "timeout"}
...
def git_head() -> str:
    if not shutil.which("git") or not (ROOT / ".git").exists(): return ""
...
def project_version() -> str:
    ...
    for line in text.splitlines():
        stripped = line.strip()
        if stripped.startswith("version") and "=" in stripped: return stripped.split("=", 1)[1].strip().strip('"\'')
    return "unknown"
...
def main() -> int:
    ...
    if args.runtime:
        ...
        checks.append(run("http_load_smoke", ["docker", "compose", "exec", "-T", "app", "python", "scripts/load_smoke.py", "--url", "http://127.0.0.1:8000", "--requests", "60", "--concurrency", "8"], timeout=max(args.command_timeout, 180)))
        if args.live_inference:
            checks.append(run("long_context_live", ["docker", "compose", "exec", "-T", "app", "python", "-m", "scripts.long_context_probe", "--context-tokens", os.environ.get("X1_DEEP_CONTEXT_TOKENS", "16384"), "--live-url", "http://llama:8080"], timeout=max(args.command_timeout, 600)))
            checks.append(run("capacity_calibration", [sys.executable, "scripts/capacity_calibrate.py", "--samples", "1"], timeout=max(args.command_timeout, 1200), env=python_env))
        else:
    ...
    payload = {"format": "x1-release-gate-v2", "status": status, "version": project_version(), "git_head": git_head(), "mode": "runtime" if args.runtime else "static", "live_inference_requested": bool(args.live_inference), "capacity_calibration_requested": bool(args.runtime and args.live_inference), "containerized_gate": containerized_gate, "historical_regression_modules": 45, "started_at": started_at, "finished_at": utcnow(), "failed_required_checks": failed_required, "checks": checks}
