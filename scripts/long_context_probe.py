...
def offline_probe(context_tokens: int, output_tokens: int) -> dict:
    ...
    messages = [ChatMessage(role="system", content="X1 release-gate long-context probe."), ChatMessage(role="user", content=payload)]
    ...
    return {"status": "passed" if total_chars <= prompt_budget_chars and "НАЧАЛО_ЗАПРОСА" in text and "КОНЕЦ_ЗАПРОСА" in text else "failed", "context_tokens": context_tokens, "output_tokens": output_tokens, "prompt_budget_chars": prompt_budget_chars, "compiled_chars": total_chars, "kept_head_marker": "НАЧАЛО_ЗАПРОСА" in text, "kept_tail_marker": "КОНЕЦ_ЗАПРОСА" in text}


async def live_probe(base_url: str, context_tokens: int, timeout: float) -> dict:
    # Keep the synthetic prompt inside the candidate tier itself. The previous
    # 36k minimum was too large for a meaningful 8K candidate calibration.
    target_chars = min(max(18_000, (context_tokens - 2048) * 3), max(18_000, context_tokens * 3))
    unit = "Контекст для проверки длинного окна модели. "
    body = (unit * (target_chars // len(unit) + 1))[:target_chars]
    prompt = f"НАЧАЛО\n{body}\nКОНЕЦ\nОтветь только словом OK."
    request = {"model": "local", "messages": [{"role": "system", "content": "Это техническая проверка окна контекста X1."}, {"role": "user", "content": prompt}], "max_tokens": 16, "stream": False, "temperature": 0}
    ...
    try:
        ...
        return {"status": "passed" if response.is_success and bool(content) else "failed", "http_status": response.status_code, "latency_ms": latency_ms, "prompt_chars": len(prompt), "context_tokens": context_tokens, "response_preview": content[:120]}
