...
def audit() -> dict:
    ...
    quality = (ROOT / "scripts/chat_quality_acceptance.py").read_text("utf-8")
    ...
    for token in ('FORMAT = "x1-build-provenance-v1"', '_BUILD_INPUTS = ("Dockerfile", "docker-compose.yml")', 'root / "build-inputs" / name', 'manifest[f"build-inputs/{name}"]'):
    ...
    for token in (
        '"x1-final-release-acceptance-v1"', "X1_LOAD_TOKENS", "X1_PRODUCTION_ADMIN_TOKEN",
        "_validate_target_transport(base_url)", "Public final acceptance requires HTTPS before Bearer load tokens are sent",
        "scripts/load_acceptance.py", "scripts/chat_quality_acceptance.py", "scripts/rc_release_candidate.py", "scripts/production_acceptance.py",
        '"accepted_for_launch": status == "passed"', "if load[\"status\"] != \"passed\"", "if quality[\"status\"] != \"passed\"", "if rc[\"status\"] != \"passed\"",
    ...
    if "shell=True" in quality or "Authorization" not in quality:
        errors.append({"code": "chat_quality_acceptance_security_contract_missing"})

    for token in (
        "scripts/final_release_acceptance.py", "X1_LOAD_TOKENS", "X1_PRODUCTION_ADMIN_TOKEN",
        "chat-quality-acceptance-latest.json", '"accepted_for_launch": true',
    ):
