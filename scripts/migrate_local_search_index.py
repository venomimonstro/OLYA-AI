...
def migrate_catalog(path: Path) -> int:
    ...
    try:
        rows = connection.execute(
            'SELECT city,osm_type,osm_id,name,category,address,phone,website FROM businesses'
        )
        for row in rows:
            ...
            try:
                store.upsert_business({
                    'name': row['name'], 'category': row['category'], 'subcategory': row['category'],
                    'city': row['city'], 'address': row['address'], 'phone': row['phone'], 'website': row['website'],
...
def migrate_live_cache(path: Path) -> int:
    ...
    try:
        ...
        if not columns:
            # Historical versions used other table names. Ignore safely instead
            # of guessing a schema and corrupting the unified index.
            return 0
