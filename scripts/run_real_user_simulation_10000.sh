...
printf '[OLYA-10K] Usage-weighted live site + AI simulation\n'
if [ "${1:-}" = "--full" ]; then
  shift
  echo '[OLYA-10K] WARNING: full 10,000 live generations on one CPU inference slot can run for many hours/days.'
  docker compose exec -T app python -m scripts.real_user_live_weighted_10000 --full "$@"
else
  docker compose exec -T app python -m scripts.real_user_live_weighted_10000 "$@"
fi
