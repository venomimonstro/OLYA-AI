...
def audit() -> dict:
    ...
    if "6-9 actionable points" not in shapes["practical_chess"]:
        errors.append("practical_depth_missing")
    if "4-7 key points" not in shapes["explain"]:
        errors.append("explain_depth_missing")
    if "recommendation first" not in shapes["compare"]:
        errors.append("comparison_shape_missing")
    if "Do not add research" not in shapes["writing"]:
        errors.append("writing_search_guard_missing")
    if "1-4 sentences" not in shapes["short"]:
        errors.append("explicit_short_not_respected")

    if tuple(SearxngDiscovery.primary_engines) != ("google", "yandex"):
        errors.append("primary_search_not_google_yandex")
    if tuple(SearxngDiscovery.fallback_engines) != ("bing", "startpage"):
        errors.append("search_fallback_order")

    settings = (ROOT / "searxng" / "settings.yml").read_text("utf-8")
    for engine in ("google", "yandex", "bing", "startpage"):
        if f"- {engine}" not in settings:
            errors.append(f"searx_engine_missing:{engine}")
    if "keep_only:" not in settings:
