...
async def _governor_burst(max_queue: int=64, tasks: int=512) -> dict[str,Any]:
    ...
    async def holder():
        async with governor.slot(): holder_ready.set(); await release_holder.wait()
    hold=asyncio.create_task(holder()); await holder_ready.wait()
    async def contender():
        try:
            async with governor.slot(): return "admitted"
        except ResourceBusyError as exc: return "shed:"+str(exc)
    contenders=[asyncio.create_task(contender()) for _ in range(tasks)]
    await asyncio.sleep(0.02); peak=governor.waiting; release_holder.set(); await hold
...
def _oversized_header_status(base_url: str) -> int:
    parsed=urlparse(base_url)
    host=parsed.hostname or "127.0.0.1"; port=parsed.port or (443 if parsed.scheme=="https" else 80)
    if parsed.scheme not in {"http",""}:
        raise RuntimeError("Protocol-level oversized-body probe supports local HTTP only")
    with socket.create_connection((host,port),timeout=5) as sock:
        request=(
            f"POST /v1/auth/login HTTP/1.1\r\nHost: {host}\r\n"
        ...
        first=b""
        while b"\r\n" not in first and len(first)<1024:
            block=sock.recv(256)
            if not block: break
            first+=block
...
def main()->int:
    ...
    args=parser.parse_args(); checks:list[Check]=[]; user_id=""; overall="passed"; error=""
    ...
    try:
        ...
        body=r.json(); user_id=body["user_id"]; token=body["access_token"]; _grant(user_id); headers={"Authorization":f"Bearer {token}"}
        ...
        project=client.post("/v1/projects",headers=headers,json={"name":"Chaos paths","description":"","instructions":""}); check("chaos_project_create",project.status_code==201,{"status":project.status_code})
        workspace=client.post("/v1/code/workspaces",headers=headers,json={"name":"paths","project_id":project.json()["id"]}); check("chaos_workspace_create",workspace.status_code==201,{"status":workspace.status_code})
        ...
        oversized_status=_oversized_header_status(args.base_url)
        check("oversized_body_rejected",oversized_status==413,{"status":oversized_status})

        statuses=[]
        for _ in range(10):
            resp=client.post("/v1/auth/login",json={"email":email,"password":"definitely-wrong-password"}); statuses.append(resp.status_code)
            if resp.status_code==429: break
        check("auth_bruteforce_throttled",429 in statuses,{"statuses":statuses})

        burst=asyncio.run(_governor_burst(max_queue=64,tasks=512)); check("bounded_inference_queue",burst["peak_waiting"]<=64 and burst["shed"]>0,burst)
