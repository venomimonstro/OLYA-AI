...
for arg in "$@"; do
  case "$arg" in
    ...
    --skip-e2e) SKIP_E2E=1 ;;
    -h|--help)
      echo "Usage: bash scripts/install.sh [--no-inference] [--doctor-only] [--skip-e2e]"
      exit 0
      ;;
...
safe_context=int(sys.argv[7]); threads=int(sys.argv[8])
...
ensure_secret('POSTGRES_PASSWORD',generated_db,{'change-me-db'})
db=values['POSTGRES_PASSWORD']
setv('X1_DATABASE_URL',f'postgresql+psycopg://x1:{db}@db:5432/x1')
...
setv('X1_LLAMA_MODEL_NAME','Qwen3-30B-A3B-Q4_K_M')
setv('X1_LLAMA_BASE_URL','http://llama:8080')
setv('X1_PROJECT_SANDBOX_BACKEND','remote')
setv('X1_PROJECT_SANDBOX_IMAGE','x1-sandbox:0.39')
setv('X1_PROJECT_SANDBOX_WORKER_URL','http://sandbox-worker:8090')
...
if new_env:
    setv('X1_MAX_CONTEXT_TOKENS',min(8192,safe_context))
    setv('X1_DEEP_CONTEXT_TOKENS',safe_context)
...
if [ "$WITH_INFERENCE" -eq 1 ]; then
  info "Downloading/verifying official Qwen3-30B-A3B Q4_K_M GGUF (resumable)"
...
info "Building X1 services"
docker compose build app sandbox-worker
...
for _ in $(seq 1 90); do
  ...
  docker compose exec -T searxng python -c "import urllib.request; urllib.request.urlopen('http://127.0.0.1:8080/search?q=x1&format=json',timeout=3).read()" >/dev/null 2>&1 && search_ok=1
...
docker compose exec -T searxng python -c "import urllib.request; urllib.request.urlopen('http://127.0.0.1:8080/search?q=x1&format=json',timeout=3).read()" >/dev/null 2>&1 || fail "SearXNG did not become ready"

info "Applying database migrations"
docker compose run --rm --no-deps app alembic upgrade head

if [ "$WITH_INFERENCE" -eq 1 ]; then
  info "Starting Qwen llama.cpp and X1"
  docker compose --profile inference up -d llama app
else
  info "Starting X1 control plane without inference"
...
info "Running hardened sandbox execution probe"
docker compose exec -T app python -m scripts.sandbox_probe

if [ "$WITH_INFERENCE" -eq 1 ]; then
  info "Running production release gate, user journey and chaos simulations"
  gate_args=(--runtime --live-inference --user-journey --chaos)
  [ "$SKIP_E2E" -eq 1 ] && gate_args=(--runtime --live-inference)
  set +e
  python3 scripts/release_gate.py "${gate_args[@]}"
  gate_status=$?
  set -e
  if [ "$gate_status" -ne 0 ]; then
    docker compose ps >&2 || true
    docker compose logs --tail=120 app llama searxng sandbox-worker >&2 || true
    fail "Production release gate failed; inspect backups/release-gate-latest.json"
  fi
else
