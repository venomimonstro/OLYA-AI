...
def main() -> int:
    ...
    _require(contract, ("classify_answer_kind", 'kind == "writing"', 'kind == "audit"', 'kind == "recommendation"', "finished text", "search rank"), "answer contract")
    _require(context, ("build_answer_contract", "set_evidence_context", "_evidence_digest", "X1 ANSWER CONTRACT"), "context compiler")
    _require(quality, ("current_evidence_context", "unsupported_claim", "contradiction", "stale_claim", "bad_inference", "_quality_clip", "EVIDENCE"), "evidence-aware critic")
    ...
    _require(tests, ("test_context_compiler_preserves_bounded_evidence_for_critic", "test_consequential_recommendation_gets_critic_and_conditional_repair_budget", "test_critic_is_evidence_aware"), "quality tests")
    ...
    print(json.dumps({"status": "passed", "quality_cases": len(cases), "categories": sorted(categories)}, ensure_ascii=False))
    return 0
